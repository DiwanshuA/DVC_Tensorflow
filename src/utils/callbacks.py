...
def create_and_save_tensorboard_callback(callback_dir, tensorboard_log_dir):
    ...
    logging.info(f"Callbacks are saved at {tb_callback_filepath}")



def create_and_save_checkpoint_callback(callback_dir, checkpoint_dir):
    checkpoint_filepath = os.path.join(checkpoint_dir, "ckpt_model.h5")
    ...
    logging.info(f"tensorboard callback is saved at {ckpt_callback_filepath}")
